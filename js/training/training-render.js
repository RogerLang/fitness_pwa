(() => {
  ...
  function workoutNumberInput({ label, ei, exerciseId, si, key, value, placeholder, decimal = false, done = false, renderToken }) {
    const fieldName = `manual-${renderToken}-${ei}-${si}-${key}`;
    return `<input class="workout-number-input" name="${App.esc(fieldName)}" aria-label="${App.esc(label)}" type="text" inputmode="${decimal ? "decimal" : "numeric"}" ${AUTOFILL_GUARD} enterkeyhint="${done ? "done" : "next"}" data-e="${ei}" data-exercise-id="${App.esc(exerciseId)}" data-s="${si}" data-k="${key}" value="${App.esc(value)}" placeholder="${App.esc(placeholder ?? "")}">`;
  }
  ...
  function renderExerciseCard(ex, ei, historyContext, workout, renderToken) {
    ...
    const sets = Draft.effectiveSetCount(exerciseId, baseSets);
    ...
    for (let si = 0; si < sets; si++) {
      ...
      const weightValue = Draft.getValue(exerciseId, si, "weight");
      const repsValue = Draft.getValue(exerciseId, si, "reps");
      const rirValue = Draft.getValue(exerciseId, si, "rir");
      const done = Draft.isCompleted(exerciseId, si);
      rows += `<div class="set-row workout-set-row${done ? " set-completed" : ""}${weighted ? "" : " bodyweight-set-row"}" data-e="${ei}" data-exercise-id="${App.esc(exerciseId)}" data-s="${si}">
        <span class="set-number">${si + 1}</span>
        ${weighted ? workoutNumberInput({ label: `第 ${si + 1} 组${loadName}`, ei, exerciseId, si, key: "weight", value: weightValue, placeholder: target.weight, decimal: true, renderToken }) : ""}
        ${workoutNumberInput({ label: `第 ${si + 1} 组次数`, ei, exerciseId, si, key: "reps", value: repsValue, placeholder: target.reps, renderToken })}
        ${workoutNumberInput({ label: `第 ${si + 1} 组 RIR`, ei, exerciseId, si, key: "rir", value: rirValue, placeholder: ex.warmup ? "" : (prev?.rir ?? "1–2"), done: true, renderToken })}
        <button type="button" class="set-complete${done ? "" : " secondary"}" data-e="${ei}" data-exercise-id="${App.esc(exerciseId)}" data-s="${si}" aria-pressed="${done}">${done ? "✓" : "完成"}</button>
      </div>`;
    }
    ...
    return `<article class="card exercise-card${ex.warmup ? " warmup-card" : ""}" data-e="${ei}" data-exercise-id="${App.esc(exerciseId)}" data-load-type="${App.esc(loadType(ex))}">
      <div class="exercise-head"><div class="exercise-title-wrap"><div class="exercise-title-line"><div class="exercise-title">${App.esc(ex.name || "未命名动作")}</div>${ex.warmup ? '<span class="badge warmup-badge">热身</span>' : ""}</div><div class="exercise-meta">${App.esc(meta)}</div></div><span class="exercise-count-chip">${sets}组</span></div>
      <div class="exercise-card-layout"><div class="exercise-summary-grid">${previousHtml(summary)}${plannedHtml(ex, workout)}</div>
        <section class="exercise-panel exercise-sets-panel${weighted ? "" : " bodyweight-sets-panel"}"><div class="exercise-panel-heading training-set-heading"><div><div class="exercise-panel-title">动作组</div><div class="exercise-panel-meta">${App.esc(setsMeta)}</div></div><div class="training-set-adjust" aria-label="临时调整本次训练组数"><button type="button" class="small secondary remove-set" data-e="${ei}" data-exercise-id="${App.esc(exerciseId)}" ${sets <= 1 ? "disabled" : ""} aria-label="本次训练减少一组">−</button><span>${sets}</span><button type="button" class="small secondary add-set" data-e="${ei}" data-exercise-id="${App.esc(exerciseId)}" aria-label="本次训练增加一组">+</button></div></div>${header}${rows}</section>
      </div>
    </article>`;
  }
  ...
  function renderPlanStatus(active) {
    ...
    const time = active.workout.confirmedAt ? new Date(active.workout.confirmedAt).toLocaleString("zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" }) : "已推送";
  ...
  function renderWorkout() {
    ...
    if (!active) {
      container.innerHTML = `<div class="card training-plan-empty"><strong>还没有待训练计划</strong><span>在计划页选择训练模板，检查系统建议并推送后，这里会直接显示那一份计划。</span><button type="button" class="go-plan-page">去制定计划</button></div>`;
      return;
    }
    const planId = String(active.workout?.planId || active.planId || "").trim();
    Draft.ensurePlan(planId);
    const historyContext = buildHistoryContext({ planId, name: active.workout.planName || active.plan?.name || "" });
  ...
  window.TrainingRenderer = Object.freeze({ renderWorkout, currentWorkoutEntry });
})();